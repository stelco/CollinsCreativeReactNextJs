...
// Type definitions for Hume chat events
...
async function main(): Promise<void> {
  ...
  if (!chatId) {
    throw new Error("NEXT_PUBLIC_HUME_CHAT_ID is not set in the environment variables.");
  }
 ...
 * @throws If the NEXT_PUBLIC_HUME_API_KEY environment variable is not set.
 */
export async function fetchAllChatEvents(chatId: string): Promise<ReturnChatEvent[]> {
  const apiKey = process.env.NEXT_PUBLIC_HUME_API_KEY;

  if (!apiKey) {
    throw new Error("NEXT_PUBLIC_HUME_API_KEY is not set in the environment variables.");
  }
 ...
 */
export function generateTranscript(chatEvents: ReturnChatEvent[]): string {
 ...
 */
export function getTopEmotions(chatEvents: ReturnChatEvent[]): Partial<EmotionScores> {
